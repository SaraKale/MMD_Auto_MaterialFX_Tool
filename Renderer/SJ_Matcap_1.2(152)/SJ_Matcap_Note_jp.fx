...
//----------------------------------------
//アルベドマットキャップ×Sphテクスチャカラー乗算
//----------------------------------------
//Matcapテクスチャを起動
// 0：無効
//1：Matcap機能を有効にし、このとき計算方式は乗算である。
// 2：Matcap機能を有効にし、この時の計算方式は加算である。
#define ALBEDO_MATCAP_ENABLE 0

//[ALBEDO_MATCAP_ENABLEが1または2の場合]ここにMatcap画像のパス名を記入します。
#define ALBEDO_MATCAP_MAP_FILE「matcap.png」

//マットキャップの影響源
//0：影響度を読み取るチャンネル画像を無効にする。
//1：影響度を読み取るチャンネル画像を有効化。
#define ALBEDO_MATCAP_MAP_AFFECT_FROM 0

//テクスチャに影響を与えるチャンネル
//0：影響度を読み取るチャンネル画像のRチャンネル。
//1：影響度を読み取るチャンネル画像のGチャンネルを読み取る。
//2：影響度を読み取るチャンネル画像のBチャンネル。
//3：影響度を読み取るチャンネル画像のAチャンネル。
//4または未定義の場合：影響度のチャンネル画像のRGBチャンネルを読み取る。
#define ALBEDO_MATCAP_MAP_AFFECT_SWIZZLE 4      // 0=r  1=g  2=b  3=a  4=rgb

//[ALBEDO_MATCAP_MAP_AFFECT_FROMが1の場合] ここに影響度のチャンネル画像のパス名を記入する。
#define ALBEDO_MATCAP_MAP_AFFECT_FILE「affect.png」

//マットキャップカラーの強度は、float3を使ってRGBを制御できる。
const float3アルベドマットキャップ＝1.0;
//ここには材質がどの程度影響を受けるかを記入する。
//この値が0に近づくほど、Matcapの影響を受けにくくなる（乗算加算に関係なく）。
// この値が1の場合、完全にMatcapの影響を受けます(乗算加算に関係なく)。
// float3を使用してRGBを制御できることに注意。
//ALBEDO_MATCAP_MAP_AFFECT_FROMが1の場合、この値は影響度のチャンネル画像と乗算されます。
const float3アルベドマットキャップ_影響＝1.0;
