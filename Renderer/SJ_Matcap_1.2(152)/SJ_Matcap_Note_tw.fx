...
//----------------------------------------
//	albedo matcap 乘算Sph貼圖色
//----------------------------------------
// 啟動Matcap貼圖
// 0：無效
// 1：啟用Matcap功能，且此時計算方式為乘算。
// 2：啟用Matcap功能，且此時計算方式為加算。
#define ALBEDO_MATCAP_ENABLE 0

// [當ALBEDO_MATCAP_ENABLE為1或2時] 在此處填寫Matcap圖片的路徑名。
...
// 2：讀取影響程度的通道圖片的B通道。
// 3：讀取影響程度的通道圖片的A通道。
// 4或未定義時：讀取影響程度的通道圖片的RGB通道。
#define ALBEDO_MATCAP_MAP_AFFECT_SWIZZLE 4      // 0=r  1=g  2=b  3=a  4=rgb

// [當ALBEDO_MATCAP_MAP_AFFECT_FROM為1時] 在此處填寫影響程度的通道圖片的路徑名。
#define ALBEDO_MATCAP_MAP_AFFECT_FILE "affect.png"

// Matcap顏色強度，可以使用float3來控製RGB。
const float3 albedoMatcap = 1.0;
// 此處填寫的是材質受影響的程度。
// 當這個值越接近0時代表越不受Matcap的影響(無論乘算加算)。
// 當這個值為1時代表完全受Matcap的影響(無論乘算加算)。
// 註意可以使用float3來控製RGB。
// 如果ALBEDO_MATCAP_MAP_AFFECT_FROM為1，那麼這個值會與影響程度的通道圖片進行乘算。
