...
適用対象：拡散反射、ハイライト、法線、スムーズ度、粗さ、金属度、アンビエントオクルージョンマップなど、複数のテクスチャ（PBR）を持つモデル。
...
4. レンダラーを選択：  
   ...
   - 任意のオプションを選択すると、関連する **.fxsub / .fx** ファイルが生成され、テクスチャファイル名に基づいて同じ名前に自動的にリネームされます。ファイル名の末尾に以下の名前が含まれている場合、fxファイルに自動的に関連するファイル名パスが追加されます。ただし、この名前は汎用的なものではなく、具体的には自分で修正する必要があります。エラーが発生した場合は、テクスチャファイル名を確認してください。または、`CustomRules.ini` にリネームのカスタムルールを追加することも可能です。
   - 既にアダプトされたルール名：
      - 拡散反射マップ d/Diffuse
      - 高光マップ s/Specular
      - 法線マップ n/Normal
      - 光沢度テクスチャ sm/Smoothness
      - 粗さマップ r/Roughness
      - メタルネスマップ m/Metalness
      - 環境光遮蔽マップ Ao/Occlusion
   - 注意しなければならないのは、MMDの言語制限のため、より多くの符号化言語をサポートすることができず、マップファイル名を英語に変更するだけで解決でき、前のパスにも他の非英語文字が表示されないように注意することです。
     ...
     - PowerShader by:角砂糖
     - これらの素材ファイルを使用する場合は、これらの著者を明記してください。
...
8. エクスポートオプション：  
   ...
   - "ID番号+材質名FX出力"：ID数字番号と材質名を組み合わせた名前の.fxファイルを出力する。    
...
これらのマテリアルファイルを使用する際には、これらの著者を明記してください。  
