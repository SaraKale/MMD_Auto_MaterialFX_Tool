...
适用于：有多类贴图（PBR）的模型，如：漫反射、高光、法线、光滑度、粗糙度、金属度、环境光遮蔽贴图等。
...
- 4、选择渲染器：
   ...
   - 选择任意选项，将会输出相关的 **.fxsub / .fx**文件，根据纹理文件名自动重命名为相同的名称，如果文件名末尾含有以下名称，将会自动在fx文件添加相关的文件名路径。当然这个名称并不是通用的，具体还是要自己修改，如果出现错误请检查贴图文件名。或者自己去增加 `CustomRules.ini` 的重命名自定义规则。
   - 已适配的规则名称：
      - 漫反射贴图 d/Diffuse
      - 高光贴图 s/Specular
      - 法线贴图 n/Normal
